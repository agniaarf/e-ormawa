...
if (is_super_admin()) {
    ...
    $total_pendaftaran = $pdo->query("SELECT COUNT(*) FROM permintaan_bergabung WHERE status='menunggu'")->fetchColumn();
    ...
    $recent_pendaftaran = $pdo->query("SELECT po.*, u.nama as user_nama, o.nama as org_nama FROM permintaan_bergabung po JOIN users u ON po.user_id = u.id JOIN organisasi o ON po.organisasi_id = o.id WHERE po.status='menunggu' ORDER BY po.created_at DESC LIMIT 5")->fetchAll();
    ...
    $pendaftaran_by_status = $pdo->query("SELECT status, COUNT(*) as total FROM permintaan_bergabung GROUP BY status")->fetchAll();
} else {
