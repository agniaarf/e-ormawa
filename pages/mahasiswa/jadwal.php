...
$firstWeekday = (int) $startOfMonth->format('w'); // 0=Sun
$monthLabel = ['Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember'][$month-1];

// Kegiatan
$kg = $pdo->prepare("SELECT k.*, o.nama as org_nama FROM kegiatan k JOIN organisasi o ON k.organisasi_id = o.id WHERE k.organisasi_id IN (SELECT organisasi_id FROM anggota WHERE user_id=? AND status='aktif')");
$kg->execute([$user_id]); $kg = $kg->fetchAll();
// Wawancara
$ww = $pdo->prepare("SELECT w.*, o.nama as org_nama FROM wawancara w JOIN pendaftaran_organisasi po ON w.pendaftaran_id = po.id JOIN organisasi o ON po.organisasi_id = o.id WHERE po.user_id = ?");
$ww->execute([$user_id]); $ww = $ww->fetchAll();

$events = [];
foreach ($kg as $k) {
    $d = (int) date('j', strtotime($k['tanggal_mulai']));
    $events[$d][] = ['type'=>'kegiatan','title'=>$k['nama'],'detail'=>$k['org_nama'],'time'=>date('H:i', strtotime($k['tanggal_mulai'])),'color'=>'bg-primary text-white'];
}
foreach ($ww as $w) {
    $d = (int) date('j', strtotime($w['jadwal']));
    $events[$d][] = ['type'=>'wawancara','title'=>'Wawancara','detail'=>$w['org_nama'],'time'=>date('H:i', strtotime($w['jadwal'])),'color'=>'bg-accent text-on-surface'];
}
    ...
    <div class="max-w-5xl mx-auto space-y-6">
        <div class="bg-white rounded-2xl border border-outline-variant shadow-card overflow-hidden">
            <div class="px-6 py-4 border-b border-outline-variant flex items-center justify-between">
                <h3 class="font-bold text-on-surface">Kalender Kegiatan</h3>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="bg-white rounded-2xl border border-outline-variant shadow-card overflow-hidden">
                ...
                <div class="divide-y divide-outline-variant max-h-80 overflow-y-auto">
                    ...
                    <div class="px-6 py-3 flex items-center justify-between hover:bg-surface-low">
                        <div>
                            <p class="text-sm font-semibold text-on-surface"><?= e($k['nama']) ?> <span class="text-on-surface-variant font-normal">— <?= e($k['org_nama']) ?></span></p>
            ...
            <div class="bg-white rounded-2xl border border-outline-variant shadow-card overflow-hidden">
                ...
                <div class="divide-y divide-outline-variant max-h-80 overflow-y-auto">
                    ...
                    <div class="px-6 py-3 flex items-center justify-between hover:bg-surface-low">
                        <div>
                            <p class="text-sm font-semibold text-on-surface">Wawancara <span class="text-on-surface-variant font-normal">— <?= e($w['org_nama']) ?></span></p>
